"""Command-line entry point for pc_assistant."""
...
app = typer.Typer(add_completion=False, help="pc_assistant — PC activity recorder")
...
def record() -> None:
    ...
    Does NOT start the HTTP API. Use `serve` for that, or run both:
        pc-assistant record & pc-assistant serve
    """
    Daemon().run_forever()
...
def search(query: str, limit: int = 10, app_name: str | None = None) -> None:
    """Query the HTTP API. The daemon must be running (`pc-assistant serve`)."""
    cfg = load_config()
...
@app.command()
def mcp() -> None:
    """Run the MCP stdio server (talks to a running pc-assistant HTTP API)."""
