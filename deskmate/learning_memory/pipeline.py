...
from .graph import edges_from_lecture_items
from .store import LearningStore
from .topics_llm import TopicHit, extract_topics_llm, topics_to_dict
...
def build_learning_enrichment(
    *,
    ...
    if use_llm_topics:
        ...
        top_names = [t.name for t in topics[:3]]
        if top_names:
            for s in sessions:
                if not s.get("topics"):
    ...
    if persist and (result.concepts or result.lecture_items or topics or sessions):
        try:
            store = LearningStore(db_file=db_path) if db_path else LearningStore()
            ids = store.upsert_concepts(result.concepts)
            ...
            store.insert_lecture_items(
                result.lecture_items, concept_ids=ids, session_ref=session_ref
            )
            if topics:
                store.upsert_topics(topics)
            edges = edges_from_lecture_items(result.lecture_items)
            store.upsert_edges(edges, ids)
            edges_out = store.list_edges(limit=40)
            if sessions:
                session_ids = store.upsert_slice_sessions(sessions, status="closed")
                store.ingest_problem_sessions(sessions, session_ids)
            due = store.seed_or_update_reviews(result.concepts, ids)
            events_out = store.list_events(kind=None, status="open", limit=20)
            persisted = True
            if verbose:
                echo_stderr(
                    f"  [learning_memory] concepts={len(result.concepts)} "
                    f"lecture={len(result.lecture_items)} topics={len(topics)} "
                    f"sessions={len(session_ids)} edges={len(edges)} "
        ...
        echo_stderr(
            f"  [learning_memory] concepts={len(result.concepts)} "
            f"lecture={len(result.lecture_items)} topics={len(topics)} persisted=0"
        )
...
def format_enrichment_prompt(
    ...
    *,
    ...
    defs = [i for i in result.lecture_items if i.kind == "definition"]
    steps = [i for i in result.lecture_items if i.kind == "step"]
    rels = [i for i in result.lecture_items if i.kind == "relation"]
    ...
    edges = edges or []
    lines.append("")
    lines.append("### Concept graph edges (cite as 图谱:先决|相关|对比|导致)")
    if edges:
        for e in edges[:12]:
            lines.append(
