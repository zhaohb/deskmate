...
def _build_guidance(
    ...
    if query_status == "no_query_matches":
        ...
        next_best = (
            "no local captures exist yet. start deskmate recording before concluding "
...
def format_summary_for_agent(summary: dict[str, Any], *, include_date: bool = False) -> str:
    ...
    from deskmate.engine.day_recap_context import format_ts_recap
    ...
    if status == "not_recording" and total_frames == 0 and audio_count == 0:
        return "(No activity data was captured in this time range. Start deskmate recording.)"
