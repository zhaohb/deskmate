...
This implements the pipe lifecycle for DeskMate: durable execution rows,
...
class PipeRuntime:
    ...
    def _run_subprocess(
        ...
        env = {
            **dict(os.environ),
            "DESKMATE_PIPE_NAME": pipe.frontmatter.name,
            "DESKMATE_PIPE_EXECUTION_ID": str(execution_id),
            "DESKMATE_PIPE_CONTEXT": str(context_path),
            "DESKMATE_OUTPUT_DIR": str(output_dir),
            "DESKMATE_API": self._api_base(),
        }
        if pipe.frontmatter.permissions.read_db:
            env["DESKMATE_DB"] = str(self.db.path)
